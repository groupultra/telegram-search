...
<template>
  <div v-if="isOpen" class="flex items-center justify-center" @keydown.esc="isOpen = false">
    <div class="w-[45%] rounded-xl bg-card dark:bg-gray-800 shadow-lg">
      <!-- 搜索输入框 -->
      <div class="flex items-center gap-2 border-b dark:border-gray-700 px-4 py-3">
        <input
          v-model="keyword"
          class="w-full text-primary-900 dark:text-gray-100 bg-transparent outline-none placeholder:text-gray-500 dark:placeholder:text-gray-400"
          placeholder="搜索消息..."
        >
        <button
      ...
      <!-- 设置栏 -->
      <div v-if="showSettings" class="border-b dark:border-gray-700 px-4 py-3">
        <slot name="settings" />
      </div>
