...
<template>
  <div
    v-if="isOpen"
    ref="dropdownRef"
    class="absolute left-0 top-full z-10 mt-2 min-w-[200px] border rounded-md bg-popover dark:bg-gray-800 p-2 shadow-lg border-border dark:border-gray-600"
  >
    <div class="flex items-center gap-3 border-b dark:border-gray-600 p-3">
      <Avatar
        :name="username"
        size="md"
      />
      <div class="flex flex-col">
        <span class="text-sm text-primary-900 dark:text-gray-100 font-medium">{{ username }}</span>
    ...
    <div class="mt-2">
      <button
        class="w-full flex items-center gap-2 rounded-md px-3 py-2 text-sm text-primary-900 dark:text-gray-100 hover:bg-neutral-100 dark:hover:bg-gray-700"
