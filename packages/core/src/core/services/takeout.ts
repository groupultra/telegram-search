...
export function createTakeoutService(emitter: CoreEmitter) {
  return function (client: TelegramClient) {
    ...
    async function initTakeout(): PromiseResult<Api.account.Takeout | null> {
      ...
      try {
        const takeout = await client.invoke(new Api.account.InitTakeoutSession({
          contacts: true,
          messageUsers: true,
          messageChats: true,
    ...
    async function finishTakeout(takeout: Api.account.Takeout, success: boolean): PromiseResult<void> {
      try {
        client.invoke(new Api.InvokeWithTakeout({
        ...
        logger.withError(error).error('Finish takeout session failed')
        return withResult(null, error)
      }
    }
      ...
      async* takeoutMessages(
        ...
        try {
          ...
          // https://core.telegram.org/api/offsets
            ...
            const result = await withRetry(async () => await client.invoke(
