...
func (cq *ChatQuery) QueryOwner() *AccountQuery {
...
func (cq *ChatQuery) WithOwner(opts ...func(*AccountQuery)) *ChatQuery {
	query := (&AccountClient{config: cq.config}).Query()
...
//
//	client.Chat.Query().
//		GroupBy(chat.FieldOwnerAccountID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Chat.Query().
//		Select(chat.FieldOwnerAccountID).
//		Scan(ctx, &v)
...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := cq.withOwner; query != nil {
		if err := cq.loadOwner(ctx, query, nodes, nil,
			func(n *Chat, e *Account) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *ChatQuery) loadOwner(ctx context.Context, query *AccountQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Account)) error {
...
func (cq *ChatQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Message)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(message.FieldInChatID)
	}
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "in_chat_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *ChatQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withOwner != nil {
			_spec.Node.AddColumnOnce(chat.FieldOwnerAccountID)
		}
	}
