...
import (
	...
	"github.com/groupultra/telegram-search/ent/migrate"
...
// QueryMessages queries the messages edge of a Account.
func (c *AccountClient) QueryMessages(a *Account) *MessageQuery {
...
// QueryChats queries the chats edge of a Account.
func (c *AccountClient) QueryChats(a *Account) *ChatQuery {
...
func (c *ChatClient) QueryOwner(ch *Chat) *AccountQuery {
...
func (c *MessageClient) QueryOwner(m *Message) *AccountQuery {
	query := (&AccountClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := m.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Account, Chat, Message, User []ent.Hook
	}
	inters struct {
		Account, Chat, Message, User []ent.Interceptor
	}
)
