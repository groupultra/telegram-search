...
func (aq *AccountQuery) QueryMessages() *MessageQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, account.MessagesTable, account.MessagesColumn),
		)
...
func (aq *AccountQuery) QueryChats() *ChatQuery {
	query := (&ChatClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, account.ChatsTable, account.ChatsColumn),
		)
		fromU = sqlgraph.SetNeighbors(aq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (aq *AccountQuery) WithMessages(opts ...func(*MessageQuery)) *AccountQuery {
	query := (&MessageClient{config: aq.config}).Query()
...
func (aq *AccountQuery) WithChats(opts ...func(*ChatQuery)) *AccountQuery {
	query := (&ChatClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withMessages; query != nil {
		if err := aq.loadMessages(ctx, query, nodes,
			func(n *Account) { n.Edges.Messages = []*Message{} },
			func(n *Account, e *Message) { n.Edges.Messages = append(n.Edges.Messages, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withChats; query != nil {
		if err := aq.loadChats(ctx, query, nodes,
			func(n *Account) { n.Edges.Chats = []*Chat{} },
			func(n *Account, e *Chat) { n.Edges.Chats = append(n.Edges.Chats, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Account, init func(*Account), assign func(*Account, *Message)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(message.FieldOwnerAccountID)
	}
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.MessagesColumn), fks...))
	}))
...
func (aq *AccountQuery) loadChats(ctx context.Context, query *ChatQuery, nodes []*Account, init func(*Account), assign func(*Account, *Chat)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(chat.FieldOwnerAccountID)
	}
	query.Where(predicate.Chat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.ChatsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
