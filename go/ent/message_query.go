...
func (mq *MessageQuery) QueryOwner() *AccountQuery {
...
func (mq *MessageQuery) WithOwner(opts ...func(*AccountQuery)) *MessageQuery {
	query := (&AccountClient{config: mq.config}).Query()
...
//		GroupBy(message.FieldOwnerAccountID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Message.Query().
//		Select(message.FieldOwnerAccountID).
//		Scan(ctx, &v)
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := mq.withOwner; query != nil {
		if err := mq.loadOwner(ctx, query, nodes, nil,
			func(n *Message, e *Account) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
...
func (mq *MessageQuery) loadOwner(ctx context.Context, query *AccountQuery, nodes []*Message, init func(*Message), assign func(*Message, *Account)) error {
...
func (mq *MessageQuery) loadChat(ctx context.Context, query *ChatQuery, nodes []*Message, init func(*Message), assign func(*Message, *Chat)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "in_chat_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withOwner != nil {
			_spec.Node.AddColumnOnce(message.FieldOwnerAccountID)
		}
		if mq.withChat != nil {
			_spec.Node.AddColumnOnce(message.FieldInChatID)
		}
	}
